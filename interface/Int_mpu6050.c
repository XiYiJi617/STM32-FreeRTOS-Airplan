...
void Int_MPU6050_calculate_offset(void)
{
    ...
    Accel_struct current_accel = {0};//当前加速度
    Accel_struct last_accel = {0};//上一次加速度
    uint8_t count = 0;
    ...
    while (count < 100)
    {
        ...
        // 判断飞机是否平稳 选用的参数过小 会造成一直无法判断为平稳，abs调取绝对值防止出现负数
    ...
    int32_t acc_x_sum = 0;
    int32_t acc_y_sum = 0;
    int32_t acc_z_sum = 0;//角速度总偏移量

    int32_t gyro_x_sum = 0;
    int32_t gyro_y_sum = 0;
    int32_t gyro_z_sum = 0;//加速度总偏移量
...
void Int_MPU6050_Init(void)
{
    ...
    // 2.1 填写角速度量程为±2000°/s，但此处出的是ADC值，并非角速度
...
//读取三轴加速度  抖动比较严重 需要零偏校准  Z轴值不为0为1g
